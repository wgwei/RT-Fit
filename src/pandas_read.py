...
def main():
    ...
    # Feature Columns
    training_input_fn = tf.estimator.inputs.pandas_input_fn(x=train_x, y=train_y, batch_size=5, shuffle=True, num_epochs=None)
    
    eval_input_fn = tf.estimator.inputs.pandas_input_fn(x=test_x, y=test_y, batch_size=5, shuffle=False, num_epochs=None)
